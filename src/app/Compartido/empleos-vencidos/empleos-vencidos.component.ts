...
@Component({
  selector: 'app-empleos-vencidos',
  templateUrl: './empleos-vencidos.component.html',
  styleUrls: ['./empleos-vencidos.component.css']
})
export class EmpleosVencidosComponent implements OnInit {
  ...
  private escribirParametrosBusquedaEnUrl(){
    ...
    this.location.replaceState('ofertas-pasadas',queryStrings.join('&'));
  }
...
/*   cargarRegistros(pagina:number, cantidadRegistrosAMostrar:number,palabraclave:string,categoria:string,empresa:string){
  ...
  cargarRegistros(pagina:number, cantidadRegistrosAMostrar:number,palabraclave:string){
    this.compartidosService.Obtener_ofertas_pasadas2(pagina,cantidadRegistrosAMostrar,palabraclave)
    .subscribe( (respuesta:HttpResponse<ofertas_publicadasDTO[]>) => {
      this.Ofertas = respuesta.body;
      this.cantidadTotalRegistros = respuesta.headers.get("cantidadTotalRegistros");
      this.OfertasOriginal = this.Ofertas;
